...
public class SavepointMetadata {
    ...
    public OperatorState getOperatorState(String uid) throws IOException {
        ...
        for (OperatorID operatorID : operatorStateIndex.keySet()) {
            if( operatorID.toString().equals(uid)) {
                return operatorStateIndex.get(operatorID).asExistingState();
            }
        }
        OperatorStateSpec operatorState = operatorStateIndex.get(uid);
